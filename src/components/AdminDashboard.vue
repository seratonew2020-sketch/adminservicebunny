...
const weeklyAttendanceData = computed(() => ({
  ...
  datasets: [{
    ...
    data: [40, 39, 42, 41, 38, 5, 2]
  }]
}))

const deptDistributionData = computed(() => ({
  labels: [t('Engineering'), t('HR'), t('Sales'), t('Marketing')],
  datasets: [{
...
const headers = computed(() => [
  ...
  { title: t('table_id'), key: 'employee_code', align: 'start', sortable: true },
  ...
  <v-container fluid class="pa-4 pa-sm-6 pa-md-8">
    ...
    <v-row class="mb-6">
      <v-col cols="12" sm="6" md="4" v-for="(item, index) in stats" :key="index">
        <v-card flat rounded="xl" class="border pa-6 d-flex align-center gap-4 card-shadow h-100" :class="theme.global.current.value.dark ? 'bg-surface-dark' : 'bg-white'">
          <v-sheet
            width="56"
            height="56"
            rounded="lg"
    ...
    <v-row class="mb-6">
      <v-col cols="12" md="8">
        <v-card class="pa-4 rounded-xl border card-shadow h-100" flat :class="theme.global.current.value.dark ? 'bg-surface-dark' : 'bg-white'">
      ...
      <v-col cols="12" md="4">
        <v-card class="pa-4 rounded-xl border card-shadow h-100" flat :class="theme.global.current.value.dark ? 'bg-surface-dark' : 'bg-white'">
          ...
          <div style="height: 300px; display: flex; justify-content: center; position: relative;">
            <Doughnut :data="deptDistributionData" :options="chartOptions" />
          </div>
        </v-card>
      </v-col>
    </v-row>
      ...
      >
        <template v-slot:item="{ item }">
           <tr class="hover-row transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-medium-emphasis">#{{ item.employee_code }}</td>
              <td class="px-6 py-4">
                  <v-chip
                    size="small"
                    rounded="lg"
