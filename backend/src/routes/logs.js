...
async function logRoutes(fastify) {
  // สร้าง API endpoint: GET http://localhost:5000/api/logs
  ...
  fastify.get("/logs/analytics", async (request, reply) => {
    try {
      const { startDate, endDate, empId, name } = request.query;
      ...
      const mtdProcessed = processAttendanceLogs(resMtd.data);
